...
public:
	...
	bool overlaps(const CAABB& vAnotherAABB) const;
	bool contain(const CAABB& vAnotherAABB) const;
	bool inside(const glm::vec3& vPoint);

	CAABB intersect(const CAABB& vAnotherAABB) const;
	CAABB scale(const glm::vec3& vExtents);
