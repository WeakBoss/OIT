...
public:
	IGUI();
	IGUI(const std::string& vName, int vExecutionOrder);
	virtual ~IGUI();
	...
	void popItemWidth();
	bool dragScalar(const std::string& vLabel, hiveGraphics::EDataType vDataType, void* vValue, float vDragSpeed, const void* vMin, const void* vMax);
	bool dragScalarN(const std::string& vLabel, hiveGraphics::EDataType vDataType, void* vValue, int vComponentsNum, float vDragSpeed, const void* vMin, const void* vMax);
