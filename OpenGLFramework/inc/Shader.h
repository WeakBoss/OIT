...
public:
	...
	GLvoid setTextureUniformValue(const std::string& vTextureUniformName, const std::shared_ptr<hiveGraphics::STexture>& vioTextureConfig);
	GLvoid setImageUniformValue(const std::shared_ptr<hiveGraphics::STexture>& vTextureConfig);
	...
	//zjm
	GLboolean link(GLsizei vCount = 0, const char** vpVaryings = nullptr, GLenum vMode = GL_SEPARATE_ATTRIBS) const;
	...
	//GLvoid changeImageUniformValue(const HiveGraphics::STexture& vTextureCongfig);
...
private:
	...
	std::map<std::string, std::tuple<int, std::shared_ptr<hiveGraphics::STexture>>> m_TextureUniformNameAndBindUnitAndTC;
	std::set<std::shared_ptr<hiveGraphics::STexture>> m_BindingImageTextureSet;
	...
	//zjm add this for glsl include
	std::string __readShaderFile(const std::string& vShaderFileName) const;
