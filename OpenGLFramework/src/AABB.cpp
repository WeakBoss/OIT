...
//************************************************************************************
//Function:
bool CAABB::overlaps(const CAABB& vAnotherAABB) const
{
...
//************************************************************************************
//Function:
bool CAABB::contain(const CAABB& vAnotherAABB) const
{
	return m_Max.x >= vAnotherAABB.m_Max.x && m_Min.x <= vAnotherAABB.m_Min.x
		&& m_Max.y >= vAnotherAABB.m_Max.y && m_Min.y <= vAnotherAABB.m_Min.y
		&& m_Max.z >= vAnotherAABB.m_Max.z && m_Min.z <= vAnotherAABB.m_Min.z;
}
...
//************************************************************************************
//Function:
CAABB CAABB::intersect(const CAABB& vAnotherAABB) const
{
	if (!this->overlaps(vAnotherAABB)) return CAABB();
	else return CAABB(max(m_Min, vAnotherAABB.getMin()), min(m_Max, vAnotherAABB.getMax()));
}

CAABB CAABB::scale(const glm::vec3& vExtents)
{
	return CAABB(m_Min * vExtents, m_Max * vExtents);
}
