...
GLvoid CModel::__processTextures(const aiMesh *vAiMesh, std::vector<SMeshTexture> &voTextures)
{
	...
	if (vAiMesh->mMaterialIndex < 0)
		return;
	aiMaterial *pAiMat = m_pScene->mMaterials[vAiMesh->mMaterialIndex];
	hiveGraphics::STexture Texture2DInSRGBSpace;
	Texture2DInSRGBSpace.isSRGBSpace = GL_TRUE;
	hiveGraphics::STexture Texture2D4Roughness;
...
//************************************************************************************
//Function:
GLvoid CModel::__loadTextureFromMaterial(aiTextureType vTextureType, const aiMaterial *vMat, const std::string& vTextureNamePrefix, std::vector<SMeshTexture>& voTextures, std::shared_ptr<hiveGraphics::STexture> vTexture2D/* = std::make_shared<HiveGraphics::STexture>()*/)
{
	_ASSERT(vMat);
