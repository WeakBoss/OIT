...
void CParticleSystem::sortParticles(glm::mat4x4 const& vViewMat) {
    ...
    {
        unsigned int const num_groups = mGetThreadsGroupCount(max_elem_count); //
    ...
    m_pComputeShaders.CalculateDp->activeShader();
    {
        ...
        unsigned int const num_groups = mGetThreadsGroupCount(m_NumAliveParticles); 
    ...
    unsigned int const num_groups = mGetThreadsGroupCount(num_threads);
