...
GLint createVAO4Cube()
{
	...
	return createVAO(CubeVertices, sizeof(CubeVertices), { 3 });
}
...
GLint createVAO4Sphere()
{
	...
	for (unsigned int y = 0; y <= SEGMENTY; ++y)
	{
		for (unsigned int x = 0; x <= SEGMENTX; ++x)
		{
			...
			float PosX = std::cos(XSegment * 2.0f * hiveGraphics::PI) * std::sin(YSegment * hiveGraphics::PI);
			float PosY = std::cos(YSegment * hiveGraphics::PI);
			float PosZ = std::sin(XSegment * 2.0f * hiveGraphics::PI) * std::sin(YSegment * hiveGraphics::PI);
...
GLint createVAO4CubeVolume()
{
	...
	return createVAO(CubeVertices, sizeof(CubeVertices), { 3, 3 });
}
...
//************************************************************************************
//Function:
GLvoid genTexture(std::shared_ptr<hiveGraphics::STexture> vioTexture)
{
	GLint TextureID;
	...
	{
	case hiveGraphics::STexture::ETextureType::Texture2D:
		...
		break;
	case hiveGraphics::STexture::ETextureType::Texture2DArray:
		...
		break;
	case hiveGraphics::STexture::ETextureType::Texture3D:
		TextureType = GL_TEXTURE_3D;
		glBindTexture(TextureType, TextureID);
		glTexImage3D(GL_TEXTURE_3D, 0, vioTexture->InternalFormat, vioTexture->Width, vioTexture->Height, vioTexture->Depth, 0, vioTexture->ExternalFormat, vioTexture->DataType, !vioTexture->pDataSet.empty() ? vioTexture->pDataSet[0] : nullptr);
		break;
	case hiveGraphics::STexture::ETextureType::TextureCubeMap:
	...
	if (vioTexture->ImageBindUnit != -1)
		glBindImageTexture(vioTexture->ImageBindUnit, TextureID, 0, GL_FALSE, 0, vioTexture->Access, vioTexture->InternalFormat);
...
{
	GLuint TextureID = vTexture->TextureID;
	GLuint TextureType = -1;
	...
	case hiveGraphics::STexture::ETextureType::Texture2D:
		TextureType = GL_TEXTURE_2D;
		glBindTexture(TextureType, TextureID);
		glTexSubImage2D(TextureType, 0, vTexture->XOffset, vTexture->YOffset, vTexture->XExtent, vTexture->YExtent, vTexture->ExternalFormat, vTexture->DataType, vTexture->pDataSet[0]);
		break;
	case hiveGraphics::STexture::ETextureType::Texture2DArray:
		break;
	case hiveGraphics::STexture::ETextureType::Texture3D:
...
//************************************************************************************
//Function:
void configureDDSTexture(const std::string& vFilePath, gli::texture &voGLITexture, std::shared_ptr<hiveGraphics::STexture>& voTexture2D)
{
...
//************************************************************************************
//Function:
void configureCommonTexture(const std::string& vFilePath, std::shared_ptr<hiveGraphics::STexture>& voTexture2D)
{
...
//************************************************************************************
//Function:
void configureHDRTexture(const std::string& vFilePath, std::shared_ptr<hiveGraphics::STexture>& voTexture2D)
{
	stbi_set_flip_vertically_on_load(true);
	GLint NumChannels = 0, ImageWidth = 0, ImageHeight = 0;
...
//{
//	auto Texture2D = std::make_shared<HiveGraphics::STexture>();
//	loadTextureFromFile(vFilePath, Texture2D);
//}

//************************************************************************************
//Function:
GLint genFBO(const std::initializer_list< std::shared_ptr<hiveGraphics::STexture>>& vioTextureAttachments)
{
	GLint FBO;
	...
	for (auto& vioTexture : vioTextureAttachments)
	{
		...
		{
		case hiveGraphics::STexture::ETextureAttachmentType::DepthTexture:
			...
			break;
		case hiveGraphics::STexture::ETextureAttachmentType::StencilTexture:
			...
			break;
		case hiveGraphics::STexture::ETextureAttachmentType::DepthAndStencilTexture:
	...
	int RenderBufferWidth = vioTextureAttachments.size() > 0 ? (*vioTextureAttachments.begin())->Width : hiveGraphics::WINDOW_KEYWORD::WINDOW_WIDTH;
	int RenderBufferHeight = vioTextureAttachments.size() > 0 ? (*vioTextureAttachments.begin())->Height : hiveGraphics::WINDOW_KEYWORD::WINDOW_HEIGHT;
...
int captureScreen2Img(const std::string& vFileName, int vQuality)
{
	...
	WindowWidth = hiveGraphics::WINDOW_KEYWORD::WINDOW_WIDTH;
	WindowHeight = hiveGraphics::WINDOW_KEYWORD::WINDOW_HEIGHT;
...
//************************************************************************************
//Function:
void blitFrameBufferTexture(std::shared_ptr<hiveGraphics::STexture> vioSrcTexture, std::shared_ptr<hiveGraphics::STexture> vioDestTexture, GLenum vFilter/* = GL_NEAREST*/, std::array<bool, 4> vChannels/* = { true, true, true, true }*/)
{
...
//************************************************************************************
//Function: trim from end (in place)
...
//************************************************************************************
//Function: trim from both ends (in place)
...
//*********************************************************************************
//FUNCTION:
void getFilePath(const std::string& vFullPath, std::string& voPathWithoutFileName)
{
